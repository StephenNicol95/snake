...
public class ThreadsController extends Thread {
		...
		for(int i = 0; i <= positions.size() - 2; i++){
			...
			if(biteItself){
				...
				if(!SCORESENT) {
					...
					int dialog = JOptionPane.showConfirmDialog(null, "You got a score of: " + score + ". Do you want to send your score to the server?");
					if(dialog == 0) {
	...
	private void stopTheGame(){
		...
		//This one sends data
		int submitScore = JOptionPane.showConfirmDialog(null, "You got a score of: " + score + "\nDo you want to submit your score to the server?");
		if(submitScore == 0) {
			String username = JOptionPane.showInputDialog("What do you want your name to be (max of 3 characters, letters will be stored as uppercase only)?");
			if(username.equals("")) {
				username = "NAN";
			...
			JOptionPane.showConfirmDialog(null, "Scores: " + Arrays.toString(LeaderboardClient.getInstance().snakeRequestLeaderboards(username + " " + score)));
		}
		while(true){
			pauser();
		}
